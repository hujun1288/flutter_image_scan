...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        //图片蒙版
        self.maskImage=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"startBtn"]];
        [self.maskImage setContentMode:UIViewContentModeScaleAspectFit];
        [self.maskImage setUserInteractionEnabled:NO];
        
        //创建transform对象  先旋转
        transform = self.maskImage.transform;
        //顺时针旋转90度
        transform=CGAffineTransformRotate(transform, M_PI_2);
        //将新获取的CGAffineTransform函数赋给ImageView
        self.maskImage.transform = transform;
        
        int margin=26 + kMarginX;
        self.maskImage.frame=CGRectMake(margin, margin, self.frame.size.width-2*margin, self.frame.size.height-2*margin);
...
- (void)setCameraType:(SNFCreditScoreCameraType)cameraType {
    ...
    switch (cameraType) {
        ...
        case PASSPORT_PERSON_INFO://护照个人信息
            [self.maskImage setImage:[UIImage imageNamed:@"passport_person_info"]];
            [self.maskImage setHidden:NO];
            break;
        case PASSPORT_ENTRY_AND_EXIT://护照出入境
            [self.maskImage setImage:[UIImage imageNamed:@"passport_entry_and_exit"]];
            //顺时针旋转90度
            transform=CGAffineTransformRotate(transform, -M_PI_2);
            //将新获取的CGAffineTransform函数赋给ImageView
            self.maskImage.transform = transform;
            ...
            [self.maskImage setHidden:NO];
            break;
        case IDCARD_POSITIVE://身份证正面
            [self.maskImage setImage:[UIImage imageNamed:@"idcard_positive"]];
            [self.maskImage setHidden:NO];
            break;
        case IDCARD_NEGATIVE://
            [self.maskImage setImage:[UIImage imageNamed:@"idcard_negative"]];
            [self.maskImage setHidden:NO];
            break;
        case HK_MACAO_TAIWAN_PASSES_POSITIVE://港澳通行证正面
            [self.maskImage setImage:[UIImage imageNamed:@"hk_macao_taiwan_passes_positive"]];
            [self.maskImage setHidden:NO];
            break;
        case HK_MACAO_TAIWAN_PASSES_NEGATIVE://
            [self.maskImage setImage:[UIImage imageNamed:@"hk_macao_taiwan_passes_negative"]];
            [self.maskImage setHidden:NO];
            break;
        case BANK_CARD://银行卡
            [self.maskImage setImage:[UIImage imageNamed:@"bank_card"]];
            [self.maskImage setHidden:NO];
            break;
        default:
            break;
